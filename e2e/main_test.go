...
func TestMain(m *testing.M) {
	...
	dir, err := os.MkdirTemp("", "relay-gateway-e2e-bin")
	if err != nil {
		panic(err)
	}
	defer func() { _ = os.RemoveAll(dir) }()
	out := filepath.Join(dir, "http-relay-gateway")
	build := exec.Command("go", "build", "-o", out, "http-relay-gateway/cmd/http-relay-gateway")
