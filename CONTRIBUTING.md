# Contributing to http-relay-gateway
...
[`README.md`](README.md) is the authoritative behavior contract: the wire
contract, provider pinning, per-provider body limits, failover, hot-reload
semantics, and what may never reach a log line or `/stats`.
[`AGENTS.md`](AGENTS.md) carries the working guidance built on top of it.
...
- Scopes name the area: `gateway`, `pool`, `config`, `cmd`, `e2e`, `docs`,
  `deps`, `ci`, `workspace`, `release`. The scope is optional; `deps` and
  `ci` exist so that dependency-automation pull requests pass the same gate
  as human ones.
...
- Anything security-shaped — the client's identity reaching the edge relay,
  a provider credential or relay URL in a log line or `/stats`, the
  unauthenticated endpoint exposed beyond the internal network:
